...
export async function GET() {
  try {
    ...
    console.error("[GET /api/social/ping] threw:", e);
    return NextResponse.json({ pings: [] });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!receiverProfileId) return NextResponse.json({ error: "receiverProfileId requis" }, { status: 400 });
    ...
    console.error("[POST /api/social/ping] threw:", e?.message);
